...
//  写入自动对焦固件 Brust mode
    ...
    do {
        state = (unsigned char)OV5640_ReadReg(0x3029);
...
void OV5640_FOCUS_AD5820_Constant_Focus(void)
{
    ...
    iteration = 300;
    do {
    	 state = (unsigned char)OV5640_ReadReg(0x3023);
        if (iteration-- == 0)
        {
//            RETAILMSG(1, (TEXT("[OV5640]AD5820_Single_Focus time out !!0x%x \r\n")), state);
            CAMERA_DEBUG("OV5640_Constant_Focus time out !! %x\n",state);
            return ;
        }   
        Delay(10);
    } while(state!=0x00); 
